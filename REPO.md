...
Canonical `difficulty:*` and `security:*` labels take precedence over automated output. The temporary `classification:*` label is transport metadata only and is removed after deterministic conversion. Malformed or conflicting classification metadata routes to `triage:invalid`.
...
`.github/workflows/issue-classification.lock.yml` is the reusable deterministic issue classifier. The `.lock.yml` filename is retained for compatibility with existing immutable caller pins; it is a normal GitHub Actions workflow and is not generated by `gh-aw`. It reads only triggering issue metadata, applies one temporary `classification:<difficulty>:<security>` label using bounded deterministic heuristics, and requires only `issues: write`. It does not use Copilot inference, `copilot-requests`, a PAT, `COPILOT_GITHUB_TOKEN`, or external AI-provider credentials. Caller workflows must route and validate the resulting temporary classification through the central metadata workflow.
...
`.github/workflows/dependabot-automerge.yml` is the reusable implementation used by repository callers. It uses the organization-installed `Gamnacken` GitHub App so Release Please pull requests trigger normal repository workflows. The workflow requests native auto-merge for the exact Release Please pull request it just created or updated using `SQUASH`; repository rulesets and merge queues remain the final merge authority.
