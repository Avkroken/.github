...
Repository `.coderabbit.yaml` files use `inheritance: true` so repository-specific settings merge with the version-controlled central baseline in `Avkroken/coderabbit/.coderabbit.yaml`. `Avkroken/coderabbit` is the dedicated central CodeRabbit configuration repository. CodeRabbit Organization Settings may supply lower-priority defaults where the inheritance chain permits them; repository-local YAML should contain only router-preserving or repository-specific overrides. `Avkroken/.github` is not a substitute for the central CodeRabbit configuration repository.

The central CodeRabbit configuration should instruct workflow reviews to evaluate Avkroken reusable-workflow SHA bumps against the exact referenced central commit and dependency closure, preserve the thin-caller contract, and avoid recommending that centralized implementation be copied into caller repositories. Router-managed repositories use label-driven CodeRabbit review: automatic review is otherwise disabled, `review:coderabbit` is the positive review trigger, Draft review is allowed, and automatic incremental review after later pushes is disabled. Repository-local `reviews.auto_review` overrides must preserve that contract unless an exception is explicitly documented. `AGENTS.md` remains the binding AI-agent policy; CodeRabbit guidance is review context, not a replacement policy source.
