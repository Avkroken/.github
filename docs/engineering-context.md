...
- Cloudflare Browser Run is reserved for `Avkroken/Jobb`. Other repositories must use non-Browser-Run execution paths; Produkter uses its stateless external Playwright fetcher while keeping Cloudflare as the control/state plane.
