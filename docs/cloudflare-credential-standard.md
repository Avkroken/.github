...
Det här dokumentet beskriver en **repository-deklarerad least-privilege-modell**. Det är inte en inventering av faktiska tokens, deras värden, resource scopes eller nuvarande Cloudflare-account-state.
...
- Faktisk providerstate måste verifieras i Cloudflare när en credential provisioneras eller felsöks.

## Logiska runtimeklasser

Skvallerbyttans publika konfiguration använder separata logical read bindings för olika providerområden. Namnen är ett kodkontrakt, inte bevis på att motsvarande token finns i ett visst konto.

- **R1** — plattforms-/resursläsning.
- **R2** — analytics-/operationsläsning.
- **R3** — security-/identityläsning.

Klasserna ska behandlas som separata least-privilege-roller. Exakta Cloudflare permission groups och resource scopes bestäms vid provisionering och ska verifieras mot aktuell Cloudflare-dokumentation/provider-UI, inte lagras här som live inventory.
...
Wrangler-deploy kan exempelvis kräva rättigheter för de resurser som faktiskt deklareras i repositoryts Wrangler-konfiguration. Om Secrets Store-bindings används måste deploymiljön ha de rättigheter som Cloudflare kräver för just den operationen.

## Secrets Store

När en Worker deklarerar Secrets Store-bindings är själva bindingnamnen publik repositorykonfiguration. Secretvärden, tokenvärden, token-ID:n, faktisk scope och provisioneringsstatus är däremot extern providerstate och ska inte dokumenteras här.
...
4. verifiera den genererade sammanfattningen innan credentialen används,
