...
pending_review=$'review\tcoderabbitai[bot]\tf0e1756122a63e70cc35a4df8b1f9c4e093afc71\tPENDING\t2026-01-01T00:15:00Z\tDraft review.'
old_pending_review=$'review\tcoderabbitai[bot]\tf0e1756122a63e70cc35a4df8b1f9c4e093afc71\tPENDING\t2026-01-01T00:00:00Z\tDraft review.'
...
assert_eq in_progress "$(printf '%s\n' "$pending_review" | coderabbit_gate_state_from_tsv "$head_sha" "$fixed_now_epoch")" "recent pending CodeRabbit review on current head remains waiting"
assert_eq missing "$(printf '%s\n' "$old_pending_review" | coderabbit_gate_state_from_tsv "$head_sha" "$fixed_now_epoch")" "old pending CodeRabbit review on current head becomes retry eligible"
assert_eq in_progress "$(printf '%s\n' "$in_progress_comment" | coderabbit_gate_state_from_tsv "$head_sha" "$fixed_now_epoch")" "current-head in-progress artifact suppresses duplicate trigger"
...
assert_eq in_progress "$(printf '%s\n%s\n' "$old_complete_comment" "$in_progress_comment" | coderabbit_gate_state_from_tsv "$head_sha" "$fixed_now_epoch")" "current in-progress state wins over stale completion"
